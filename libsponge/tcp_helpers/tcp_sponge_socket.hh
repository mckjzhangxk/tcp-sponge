...
class TCPSpongeSocket : public LocalStreamSocket {
  private:
    ...
    //默认监听 _datagram_adapter，_thread_data的IN,OUT事件
    EventLoop _eventloop{};

    //condition返回false或者 eventloop返回EXIT的时候函数才会返回
    // 循环里调用_eventloop.wait_next_event(TCP_TICK_MS);
    // _eventloop _tcp_thread,_datagram_adapter的IN,OUT事件
    
    //退出循环的条件：
    // 1.condition 返回false
    // 2._eventloop中的规则全部被清空
    // 3._eventloop监听的某个fd发生错误
    ...
    // 调用_tcp_loop(true)，直到 ？？？才返回
    
    //退出循环的条件：
    // 1._eventloop监听的某个fd发生错误
    // 2._eventloop把_datagram_adapter，_thread_data 的规则全部都移除
    void _tcp_main();
  ...
  public:
    ...
    void wait_until_closed();
     ...
     //客户端的函数，在完成三次握手前，一直处于loop,完成后启动_tcp_thread
