...
bool TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    if (num_of_ack>0&&_outstanding_segs.size() > 0)//去掉有错？
        _timer.start();
    return true;
}
...
void TCPSender::tick(const size_t ms_since_last_tick) {
    if (_timer.expire(ms_since_last_tick)) {// 当发现 超时的时候，重传_outstanding_segs的tcpsegment
        ...
        if (_fin_send && (_ackno + seg.length_in_sequence_space() ==
                          (_next_seqno - 1))) {  // _next_seqno-1是FIN的索引(当_fin_send=true时)
            seg.header().fin = true;
        }
        _transmit(seg, false);
    }
...
void TCPSender::_transmit(TCPSegment &seg, bool is_new_segment) {
